...
==================================================================================================================
  ...
  My settings
  veh = [this, 10, 0, 0, FALSE, FALSE] execVM "vehicle.sqf"
...
_createCrew = if (count _this > 6) then {_this select 6} else {false};
_disableCrewDamage = if (count _this > 7) then {_this select 7} else {false};
_mccEvac = if (count _this > 8) then {_this select 8} else {false};
_unitinit = if (count _this > 9) then {_this select 9} else {};
...
{    
    ...
    {    
		...
		//Check if crew should be created
		if (_createCrew) then {
			createVehicleCrew _unit;
		};
		
		//Disable crew damage
		if (_disableCrewDamage) then {
			_crew = crew _unit;
		...
		//Set as mcc evac
		if (_mccEvac) then {
			[_unit, west, true, true] remoteExec ["MCC_fnc_setEvac",2];
		};
        ...
        if ((_rounds == _respawns) && !(_noend)) then {_run = false;};
    };
};
	...
	{
		{
			...
			//Buildings Cleared
			_clearedBuildings = count(allMapMarkers select {markerColor _x == "colorBLUFOR"});
			_occupiedBildungs = count(allMapMarkers select {markerColor _x == "colorOPFOR"});
			_progress = (_clearedBuildings / ((_clearedBuildings + _occupiedBildungs) max 1)) * 100;

			//Aggression
			_currAggro = round(missionNamespace getVariable ["Aggression", 0]);
			
			//texts
			_cbText = format["Cleared Sectors: %1",_clearedBuildings];
			_obText = format["Occupied Sectors: %1",_occupiedBildungs];

...
while {true} do 
{
	...
	{
		[_autoMarkerTag, format["Active Markers: %1", _activeMarkers]] spawn bia_to_log;
	...
	if (count _westArr > 0) then 
	{
		...
		if (count _closeMarkers > 0 && !(missionNamespace getVariable ["SideMissionActive", false])) then 
		{
			...
			if (((count _currActiveEnemies) + _maxAddPerSector) <= _maxEnemies) then 
			{
				...
				[_autoMarkerTag, format["Activating Marker (%1)", _closestMarker]] spawn bia_to_log;
				
				missionNamespace setVariable ["ReduceEnemies", false, true];
				[_autoMarkerTag, format["Unblocked spawning of random Units"]] spawn bia_to_log;
			};
		};
		...
		) then
		{
			...
			[_autoMarkerTag, format["Blocked spawning of random Units"]] spawn bia_to_log;
		};
		...
		{
			[_autoMarkerTag, "Deactivating far sectors"] spawn bia_to_log;
	...
	} else 
	{
		...
		{
			[_autoMarkerTag, "Deactivating all sectors"] spawn bia_to_log;

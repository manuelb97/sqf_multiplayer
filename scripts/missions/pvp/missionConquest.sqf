...
["PvP_Conquest", "Mission Prep started"] spawn bia_to_log;
...
_zoneCenters = [_aoRadius, _zoneRadius, _numZones, _urbanZoneMinBuildings, _vegeZoneMinTrees, _minZoneDistance, _maxZoneDistance] call bia_pvp_location_conquest;

{
	_zoneCenter = _x;
	_areaMarker = format["Zone_%1", _forEachIndex];
	createMarker [_areaMarker, _zoneCenter];
	_areaMarker setMarkerShapeLocal "ELLIPSE";
	_areaMarker setMarkerColorLocal "ColorRed";
	_areaMarker setMarkerAlphaLocal 0.25;
	_areaMarker setMarkerSize [_zoneRadius, _zoneRadius];
} forEach _zoneCenters;
...
["PvP_Conquest", _text] spawn bia_to_log;
...
{
	_player = _x;
	_player setPos (_defenderArsenal getPos [1, random 360]);
	_player setDir (_player getDir _defenderArsenal);
...
while {missionNamespace getVariable ["PvPMissionActive", false]} do 
{
	...
	_defendUnits = allUnits select {_x getVariable ["pvp_defend", false] && _x distance _missionLocation < _radius};
	_defendUnits = _defendUnits + (_defenders select {_x distance _missionLocation < (_radius + 100)});

...
["BlackfishCASDebug", format["%1", str [_weaponsToFire, _weaponMuzzleMagazineIdxList]], true] spawn bia_to_log;
...
{
	["BlackfishCASDebug", format["%1", str [_aircraft, _selectedTarget, _weaponMuzzleMagazineIdxList select _weaponsToFireIdx, _customOffset]], true] spawn bia_to_log;

	if (_i > 1) then 
	{
		_newTargetPos = [(_pos select 0) + random [-30,0,30], (_pos select 1) + random [-30,0,30], _pos select 2];
